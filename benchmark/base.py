...
import flag_train
from flag_train.utils import shape_utils
...
torch_backend_device = flag_train.runtime.torch_backend_device
torch_device_fn = flag_train.runtime.torch_device_fn
device = flag_train.device
vendor_name = flag_train.vendor_name
if device == "musa":
...
class Benchmark:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.train_op = kwargs.get("train_op", None)
    ...
    def init_user_config(self):
        ...
        if vendor_name == "kunlunxin":
            Config.shape_file = os.path.join(
                ...
                "../src/flag_train/runtime/backend/_kunlunxin/core_shapes.yaml",
            )  # Speed Up Benchmark Test, Big Shape Will Cause Timeout
        elif vendor_name == "enflame":
            Config.shape_file = os.path.join(
                os.path.dirname(__file__),
                "../src/flag_train/runtime/backend/_enflame/core_shapes.yaml",
            )
        self.set_shapes(Config.shape_file)

    def set_train(self, train_op):
    ...
    def _measure_input(self, input, case_id=None):
        ...
        try:
            ...
            if "latency" in self.to_bench_metrics:
                if self.train_op:
                    metric.latency = self.get_latency(self.train_op, *args, **kwargs)
                else:
                    if self.op_name == "zero_":
                        with flag_train.use_train():
                            metric.latency = self.get_latency(
                                self.torch_op, *args, **kwargs
                            )
                    else:
                        # exclude flagtrain' zero_ to avoid the overhead of zero_
                        # in do_bench's clear_cache
                        with flag_train.use_train(exclude=["zero_"]):
                            metric.latency = self.get_latency(
                                self.torch_op, *args, **kwargs
                            )
...
class TexGluBenchmark(Benchmark):
    ...
    def init_user_config(self):
        ...
        if not supported:
            pytest.skip(
                "No geglu shapes satisfy the constraints of FlagTrain implementation."
            )
