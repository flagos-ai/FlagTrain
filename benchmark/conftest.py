...
import flag_train
from flag_train.cli_override import add_override_arguments, apply_overrides_from_args
...
recordLogger = logging.getLogger("flag_train_benchmark")
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    if Config.list_cases:
        with open(REPORT_FILE, "w") as f:
            json.dump(
                {
                    "schema_version": "flagtrain.benchmark-case-list/v2",
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if unknown or not_executed:
        ...
        if reporter:
            reporter.write_line(
                f"FlagTrain case selection failed: unknown={unknown}; not_executed={not_executed}"
            )
