...
try:  # pragma: no cover - best effort fallback
    from flag_train import runtime as _runtime
...
# set FLAGTRAIN_SOURCE_DIR for cpp extension to find
os.environ["FLAGTRAIN_SOURCE_DIR"] = str(Path(__file__).parent.resolve())
...
if use_env_c_extension and not _has_cpp_so:
    warnings.warn(
        "[FlagTrain] USE_C_EXTENSION is set, but C extension is not available. "
...
if use_env_c_extension and _has_cpp_so:
    try:
        from flag_train import c_operators as _co

        c_operators = _co
        has_c_extension = True
        from flag_train import aten_patch
