...
"""Command-line orchestration for offline FlagTrain Pretune runs.

The command validates a combined FlagTree/FlagTrain operator YAML, selects
...
Environment variables:
  * ``FLAGTRAIN_DB_URL`` is the inherited database URL when ``--database`` is
    absent; ``FLAGTRAIN_CACHE_DIR`` chooses the default SQLite cache directory.
  * Backend visibility variables such as ``CUDA_VISIBLE_DEVICES``,
    ...
    the run manifest.
  * ``USE_FLAGTUNE`` selects FlagTrain' Default or FlagTune routing, while
...
from flag_train.flagtune.collection.scheduler import (  # noqa: E402
...
from flag_train.flagtune.contracts.operator import (  # noqa: E402
    OperatorBenchmarkSpec,
    OperatorConfigError,
    initialize_planning_context,
    load_operator_benchmark_spec,
)
from flag_train.flagtune.contracts.records import (  # noqa: E402
    PlanningContext,
    ShapeRecord,
)
from flag_train.flagtune.reporting.artifacts import (  # noqa: E402
    PretuneIOError,
    combine_logs,
    load_shape_config,
    make_run_dir,
...
from flag_train.flagtune.reporting.schema import SCHEMA_VERSION  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Pretune FlagTrain shapes with the policy selected by the current "
            "FlagTrain/FlagTree environment."
        ),
    ...
    parser.add_argument(
        "--database",
        help="SQLite database file. Overrides inherited FLAGTRAIN_DB_URL.",
    )
    parser.add_argument(
        "--parallel",
        type=int,
...
def default_database_url(
    ...
    Precedence is ``--database``, inherited ``FLAGTRAIN_DB_URL``, then the
    vendor/Triton-specific FlagTrain SQLite cache path.
    """
    if args.database:
    ...
    inherited = os.environ.get("FLAGTRAIN_DB_URL")
    if inherited:
        kind, path = parse_sqlite_url(inherited)
        return DatabasePlan(inherited, kind, path, "FLAGTRAIN_DB_URL")
    cache_root = Path(os.environ.get("FLAGTRAIN_CACHE_DIR", Path.home() / ".flagtrain"))
    path = (
    ...
    return DatabasePlan(_sqlite_url(path), "sqlite", path, "FlagTrain default")
