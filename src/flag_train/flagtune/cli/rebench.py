...
``--device`` therefore restricts this process to one device before the FlagTrain
...
# Importing anything under ``flag_train`` runs its package initializer, which
...
def _pin_single_device(token: str) -> str:
    ...
    Limitations:
        This must run before the FlagTrain runtime is imported.  The backend
...
def _rewrite_latency(
    ...
    *,
    ...
    from flag_train.flagtune.reporting.schema import format_ms
...
def measure_all(
    *,
    ...
    """
    from flag_train.flagtune.contracts.operator import OperatorConfigError
    from flag_train.flagtune.runtime.device import DeviceProbeError
...
def summarize(
    *,
    ...
    """
    from flag_train.flagtune.reporting.schema import rounded_derived, rounded_ms
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    from flag_train.flagtune.cli.pretune import PretuneError, load_shape_records
    from flag_train.flagtune.contracts.operator import (
        OperatorConfigError,
        load_operator_benchmark_spec,
    )
    from flag_train.flagtune.runtime.device import (
        DeviceProbeError,
