...
The model-facing ``variants`` section is compiled by FlagTree.  The FlagTrain
...
Environment variables:
    ``FLAGTRAIN_DB_URL`` is temporarily replaced with ``sqlite:///:memory:`` by
...
@dataclass(frozen=True)
class BenchmarkSpec:
    """Describe tensor construction and a public FlagTrain invocation.
...
class OperatorBenchmarkSpec:
    ...
    FlagTrain-only shape and benchmark schema, plus a source hash that workers use
    ...
    def public_operator_name(self) -> str:
        """Derive the trusted public FlagTrain callable name from ``op_id``.

        Returns:
            A Python identifier resolved from a code-owned override or the last
            path segment of a ``flagtrain/...`` identity.

        Raises:
            OperatorConfigError: If the namespace is not handled by FlagTrain.
        """
...
def public_operator_name(op_id: str) -> str:
    """Resolve a FlagTrain public callable name without YAML-provided imports."""
    try:
    ...
    if parts[0] != "flagtrain":
        raise OperatorConfigError(
            f"unsupported FlagTune op_id namespace {parts[0]!r}; expected 'flagtrain'"
        )
    name = _PUBLIC_OPERATOR_OVERRIDES.get(validated, parts[-1])
    return _name(name, f"public operator for {validated!r}")


def resolve_public_operator(flag_train_module: Any, op_id: str) -> Any:
    """Return the trusted public FlagTrain callable derived from ``op_id``."""
    name = public_operator_name(op_id)
    operator = getattr(flag_train_module, name, None)
    if not callable(operator):
        raise OperatorConfigError(
            f"flag_train has no public callable {name!r} for op_id {op_id!r}"
        )
    return operator
...
def _parse_benchmark(
    ...
    Security:
        ...
        ``flag_train_public``.  YAML cannot name Python modules, arbitrary
    ...
    if invoke.get("kind") != "flag_train_public":
        raise OperatorConfigError(f"{location}.invoke.kind must be 'flag_train_public'")
...
def load_operator_benchmark_spec(path: str | Path) -> OperatorBenchmarkSpec:
    ...
    Args:
        path: Filesystem path to the combined FlagTree model and FlagTrain
...
def initialize_planning_context(
    ...
    """Initialize backend metadata for a configured public FlagTrain operator.
    ...
    Implementation:
        Initialization temporarily points ``FLAGTRAIN_DB_URL`` at an in-memory
    ...
    Limitations:
        This call imports FlagTrain and initializes the registered device
        backend. It discovers visibility but does not inspect occupancy or
        reserve a device.
    """
    original_db_url = os.environ.get("FLAGTRAIN_DB_URL")
    os.environ["FLAGTRAIN_DB_URL"] = "sqlite:///:memory:"
    try:
        import triton

        import flag_train
        from flag_train.flagtune.runtime.device import probe_flagtune_environment
        from flag_train.runtime.backend import _state

        resolve_public_operator(flag_train, spec.op_id)
        ...
        if isinstance(exc, OperatorConfigError):
            raise
        raise OperatorConfigError(
            f"cannot initialize FlagTrain planning metadata: {exc}"
        ) from exc
    finally:
        if original_db_url is None:
            os.environ.pop("FLAGTRAIN_DB_URL", None)
        else:
            os.environ["FLAGTRAIN_DB_URL"] = original_db_url
