...
"""Logging helpers for flag_train.

Notes
-----
1) When you enter through the public APIs `enable`, `only_enable`, or the
    context manager `use_train`, the `record` flag controls whether op-level
    logging is enabled and where it is written.
2) If you import `flag_train` and call operators directly (e.g., `flag_train.mm`)
    without those helpers, call `setup_flagtrain_logging()` yourself to initialize
...
def _remove_file_handlers(logger: logging.Logger):
    # Remove and close only the FileHandlers created by setup_flagtrain_logging.
    # This avoids touching unrelated FileHandlers attached by other modules.
    removed = False
    for h in list(logger.handlers):
        if isinstance(h, logging.FileHandler) and getattr(h, "_flagtrain_owned", False):
            h.close()
...
def setup_flagtrain_logging(path=None, record=True, once=False):
    logger = logging.getLogger("flag_train")
    ...
    filename = Path(path or Path.home() / ".flagtrain/oplist.log")
    handler = logging.FileHandler(filename, mode="w")
    handler._flagtrain_owned = True
...
def teardown_flagtrain_logging(logger: logging.Logger | None = None):
    """Remove file handlers for the flag_train logger (used on context exit)."""

    logger = logger or logging.getLogger("flag_train")
    _remove_file_handlers(logger)
...
def setup_precision_logging(path=None):
    ...
    filename = Path(path or Path.home() / ".flagtrain/precision.log")
...
def get_call_location():
    for frame in traceback.extract_stack():
        if "flag_train" not in frame.filename and "torch" not in frame.filename:
