...
try:
    ...
    from flag_train.flagtune.runtime._benchmark_protocol import (
        BenchmarkMode,
        BenchmarkProtocol,
        resolve_benchmarker,
    )
...
from flag_train import runtime
from flag_train.flagtune import cost_model
from flag_train.runtime import device, torch_device_fn
from flag_train.runtime.backend import _state
from flag_train.utils.code_cache import config_cache_dir
from flag_train.utils.models import PersistantModel, SQLPersistantModel
...
FLAGTRAIN_DB_URL = os.getenv("FLAGTRAIN_DB_URL", None)
...
libcache = LibCache(FLAGTRAIN_DB_URL)


class LibTuner(triton.runtime.Autotuner):
    """`LibTuner` is the base class for `FlagTrain` library autotuner.
    ...
    def __init__(
        ...
        Args:
            flagtune_op_name: FlagTrain legacy expanded-config enablement key.
                It never participates in FlagTree bundle identity. ``None``
                disables legacy config-space switching for this tuner.
            flagtune_expand_op_name: Optional FlagTrain name used to load the
            ...
            flagtune_yaml_path: Optional FlagTrain expanded-config YAML path used
...
def libtuner(
    ...
    FlagTune Args:
        ...
        flagtune_op_name: FlagTrain legacy runtime enablement key.
        flagtune_expand_op_name: Independent legacy expanded-config name; it
            defaults to ``flagtune_op_name``.
        flagtune_op_id: Globally namespaced FlagTree operator identity.
        flagtune_variant: Single-segment implementation/model variant.
        flagtune_yaml_path: FlagTrain expanded-config YAML, not the operator
            registration config.
    ...
    Notes:
        ...
        for FlagTree model loading while ``flagtune_op_name`` retains legacy FlagTrain
        enablement semantics. Missing metadata or integration failures fall
        back to the default policy.  Enabling FlagTrain' legacy expanded search
...
def find_flagtune_benchmark_target(
    ...
    Args:
        public_operator: Callable exported from the trusted ``flag_train`` public
    ...
    Notes:
        ...
        public callable already selected by FlagTrain runtime dispatch.
    """
