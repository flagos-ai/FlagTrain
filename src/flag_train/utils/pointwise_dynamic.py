...
from flag_train.utils.code_cache import code_cache_dir
from flag_train.utils.code_utils import IndentedBuffer, write_atomic
from flag_train.utils.codegen_config_utils import CodeGenConfig, get_codegen_config
from flag_train.utils.device_info import get_device_capability
from flag_train.utils.shape_utils import (
    MemOverlap,
    all_c_contiguous,
    all_the_same_shape,
    all_the_same_stride,
...
from flag_train.utils.tensor_wrapper import StridedBuffer
from flag_train.utils.type_utils import ELEMENTWISE_TYPE_PROMOTION_KIND, type_promotion
...
class KernelGenerator:
    ...
    def gen_body_one_tile_per_cta_with_bptr(self, code):
        ...
        for i in range(schema.num_input_tensors()):
            ...
            if flag_train.vendor_name == "spacemit":
...
class ModuleGenerator:
    ...
    def _collect_jit_deps(scalar_fn):
        ...
        ALREADY_IMPORTED = {
            ...
            "triton",
            "triton.language",
            "flag_train.utils.shape_utils",
            "flag_train.utils.tensor_wrapper",
            "flag_train.utils.libentry",
            "flag_train.utils",
            "flag_train.runtime",
            "flag_train.utils.pointwise_dynamic",
        }
        extra_imports = {}
    ...
    def generate_imports(self, code: IndentedBuffer) -> IndentedBuffer:
        ...
        code.newline()
        code.writeline("from flag_train.utils.shape_utils import (")
        code.writeline("    heuristics_for_tile_size,")
        code.writeline("    heuristics_for_num_warps,")
        code.writeline("    stride_order,")
        code.writeline(")")
        code.writeline("from flag_train.utils.tensor_wrapper import StridedBuffer")
        code.writeline("from flag_train.utils.libentry import libentry")
        code.writeline("from flag_train.runtime import torch_device_fn")
        if self.config.balance_grid:
            code.writeline(
                "from flag_train.utils.pointwise_dynamic import _balanced_grid_partition"
            )
...
class PointwiseDynamicFunction:
    ...
    The generated code are written out to the cache directory (defaults to ~/.flagtrain).
    """
