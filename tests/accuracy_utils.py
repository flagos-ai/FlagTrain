...
fp64_is_supported = flag_train.runtime.device.support_fp64
bf16_is_supported = flag_train.runtime.device.support_bf16
int64_is_supported = flag_train.runtime.device.support_int64


def TestForwardOnly():
    return flag_train.vendor_name in []
...
def train_assert_close(res, ref, dtype, equal_nan=False, reduce_dim=1, atol=1e-4):
    res = to_cpu(res, ref)
    flag_train.testing.assert_close(
        res, ref, dtype, equal_nan=equal_nan, reduce_dim=reduce_dim, atol=atol
    )


def train_assert_equal(res, ref, equal_nan=False):
    res = to_cpu(res, ref)
    flag_train.testing.assert_equal(res, ref, equal_nan=equal_nan)
